    ...
    public static class FileHelper
    {
        ...
        public static void ErrorLog(string exMessage, string exMethod, int userID)
        {
            try
            {
                ...
                File.AppendAllText(errPath, "{userID:" + userID + ",exMedthod:" + exMethod + ",exMessage:" + exMessage + "}");
            }
            catch
            {
        ...
        {
            string errPath = HttpContext.Current.Server.MapPath(string.Format("/Log/Debug/{0}.html", DateTime.Now.ToShortDateString()));
            if (!Equals(expectValue, null))
            {
                File.AppendAllLines(errPath, new[] { string.Format("【{0}】[{3}] 实际值:<span style='color:blue;'>{1}</span> 预期值: <span style='color:gray;'>{2}</span><br/>", DateTime.Now.ToShortTimeString(), factValue, expectValue, Equals(expectValue, factValue) ? "<span style='color:green;'>成功</span>" : "<span style='color:red;'>失败</span>") });
            }
            else
            {
                File.AppendAllLines(errPath, new[] { string.Format("【{0}】[{3}] 实际值:<span style='color:blue;'>{1}</span> 预期值: <span style='color:gray;'>{2}</span><br/>", DateTime.Now.ToShortTimeString(), factValue, "空", "<span style='color:green;'>成功</span>") });
            }
        }
        #endregion
    }
